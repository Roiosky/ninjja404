...
class SppagebuilderAddonStartuplanding_webinar extends SppagebuilderAddons
{
    ...
    public function render()
{
    ...
    if ($button_icon_position == 'left') {
        $button_text = ($button_icon)
            ? '<i class="fa ' . $button_icon . '" aria-hidden="true"></i> ' . $button_text
            : $button_text;
    } else {
        $button_text = ($button_icon)
            ? $button_text . ' <i class="fa ' . $button_icon . '" aria-hidden="true"></i>'
            : $button_text;
    }

    // Inline media path handling for selector_image
    $selector_image = '';
    if (isset($settings->selector_image)) {
        // Could be an object with ->src or just a string
        if (is_object($settings->selector_image)) {
            $selector_image = $settings->selector_image->src;
        } elseif (is_string($settings->selector_image)) {
            $selector_image = $settings->selector_image;
        }
        // If it's not an absolute URL, prepend JURI::base()
        if (
            strpos($selector_image, 'http://') !== 0 &&
            strpos($selector_image, 'https://') !== 0 &&
            !empty($selector_image)
        ) {
            $selector_image = JURI::base() . $selector_image;
        }
    }

    // Inline media path handling for modal_content_image
    $modal_content_image = '';
    if (isset($settings->modal_content_image)) {
        if (is_object($settings->modal_content_image)) {
            $modal_content_image = $settings->modal_content_image->src;
        } elseif (is_string($settings->modal_content_image)) {
            $modal_content_image = $settings->modal_content_image;
        }
        if (
            strpos($modal_content_image, 'http://') !== 0 &&
            strpos($modal_content_image, 'https://') !== 0 &&
            !empty($modal_content_image)
        ) {
            $modal_content_image = JURI::base() . $modal_content_image;
        }
    }
    ...
    // Handle image content
        ...
        $url = '#' . $modal_unique_id;
        // We have $modal_content_image from above
    ...
    $output .= '<div class="sl-addon-webinar ' . $class . ' ' . $alignment . '">';
    $output .= '<div class="d-flex justify-content-between align-items-center">';

    // Webinar user avatars
    if (isset($settings->sp_startuplanding_webinar_user) && count((array)$settings->sp_startuplanding_webinar_user)) {
        $output .= '<div class="sl-users-group">';
        foreach ($settings->sp_startuplanding_webinar_user as $key => $user) {
            if ($user->avatar) {
                // Inline handle avatar path
                $avatar_path = '';
                if (is_object($user->avatar)) {
                    $avatar_path = $user->avatar->src;
                } elseif (is_string($user->avatar)) {
                    $avatar_path = $user->avatar;
                }
                if (
                    strpos($avatar_path, 'http://') !== 0 &&
                    strpos($avatar_path, 'https://') !== 0 &&
                    !empty($avatar_path)
                ) {
                    $avatar_path = JURI::base() . $avatar_path;
                }

                $output .= '<img src="' . $avatar_path . '" data-toggle="tooltip" data-placement="top" title="' . $user->name . '" alt="' . $user->name . '">';
            }
        }
        $output .= '</div>';
    }
    ...
    // Duration
    if ($duration) {
        $output .= '<span class="webinar-duration">' . $duration . '</span>';
    }

    $output .= '</div>'; // close .d-flex
    ...
    if ($modal_selector == 'image') {
        if ($selector_image) {
            ...
            $output .= '<img src="' . $selector_image . '" alt="' . $alt_for_button . '">';
        ...
        $output .= '<a class="sppb-btn ' . $button_class . ' sppb-magnific-popup sppb-modal-selector" ' . $attribs . ' href="' . $url . '" id="' . $modal_unique_id . '-selector">';
        $output .= $button_text;
        $output .= '</a>';
    }

    $output .= '</div>'; // close .sl-addon-webinar
    ...
    #>
        <div id="{{ modalUniqueId }}" class="mfp-hide popup-image-block">
            <div class="modal-inner-block">
                <# if(data.modal_content_image && typeof data.modal_content_image === "string" && data.modal_content_image.indexOf("https://") == -1 && data.modal_content_image.indexOf("http://") == -1){ #>
    ...
    <div class="sl-addon-webinar {{ data.class }} {{ data.alignment }}">
            <div class="d-flex justify-content-between align-items-center">
            <# if(data.sp_startuplanding_webinar_user){ #>
                <div class="sl-users-group">
                <# _.each(data.sp_startuplanding_webinar_user, function (user, key) { 
                    let avatar_image = "";
                    if(user.avatar){
                        if(typeof user.avatar === "object" && typeof user.avatar.src === "string"){
                            ...
                            // Fallback to a default avatar if user.avatar is neither string nor object with src
                            avatar_image = `src=${pagebuilder_base + "path/to/default/avatar.png"}`; // Replace with your default avatar path
                        }
                    } else {
                        // Fallback to a default avatar if user.avatar is undefined or null
                        avatar_image = `src=${pagebuilder_base + "path/to/default/avatar.png"}`; // Replace with your default avatar path
                    }
                #>
            ...
            <# }
            if(data.duration){ #>
            <span class="webinar-duration">{{data.duration}}</span>
            <# } #>
            </div>
            <# if(data.modal_selector == "image") { #>
                <a class="sppb-modal-selector sppb-magnific-popup" {{{ attribs }}} href=\'{{ modalUrl }}\' id="{{ modalUniqueId }}-selector">
                    <# if(data.selector_image && typeof data.selector_image === "string" && data.selector_image.indexOf("https://") == -1 && data.selector_image.indexOf("http://") == -1){ #>
            ...
            <# } else { #>
                <a class="sppb-btn {{ buttonClass }} sppb-magnific-popup sppb-modal-selector" {{{ attribs }}} href=\'{{ modalUrl }}\' id="{{ modalUniqueId }}-selector">
                    <# if( buttonIconPosition == "left" && data.button_icon ) { #>
                        <i class="fa {{ data.button_icon }}"></i>
                    <# } #> 
                    {{ data.button_text }} 
                    <# if( buttonIconPosition == "right" && data.button_icon ) { #>
                        <i class="fa {{ data.button_icon }}"></i>
                    <# } #>
                </a>
            <# } #>
