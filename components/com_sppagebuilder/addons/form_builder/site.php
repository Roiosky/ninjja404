        ...
        if (isset($settings->sp_form_builder_item) && is_array($settings->sp_form_builder_item)) {
            ...
            foreach ($settings->sp_form_builder_item as $item_key => $item_value) {
                ...
                if ($field_type == 'radio') {
                    ...
                    $output .= '<input type="number" id="' . $item_name_id . '" name="form-builder-item-[' . $field_name . '' . ($field_is_required ? '*' : '') . ']" class="sppb-form-control"' . ($number_min != '' ? ' min="' . $number_min . '"' : '') . '' . ($number_max ? ' max="' . $number_max . '"' : '') . '' . ($number_step ? ' step="' . $number_step . '"' : '') . '' . ($field_placeholder ? ' placeholder="' . $field_placeholder . '"' : '') . '' . ($field_is_required ? ' aria-required="true" required' : '') . '>';
                    $output .= $field_is_required ? '<span class="sppb-form-builder-required">' . $required_field_message . '</span>' : '';
                    $output .= '</div>'; //.sppb-form-group
